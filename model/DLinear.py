...
class DlinearModel(nn.Module):
    ...
    """
    def __init__(self,  num_timesteps_input, num_timesteps_output, num_features, dropout=0.1):
        super(DlinearModel, self).__init__()
        self.num_features = num_features
        self.num_timesteps_input = num_timesteps_input
        self.hidden = num_timesteps_input
        self.num_timesteps_output = num_timesteps_output
        self.decomposition = series_decomp(kernel_size=3) 
        # Single Linear layer to handle all features
        self.Linear1 = nn.Linear(num_timesteps_input * num_features, num_timesteps_output * num_features)
        self.Linear2 = nn.Linear(num_timesteps_input * num_features, num_timesteps_output * num_features)
    ...
    def forward(self, x, time=None, dec_time=None, mask=None):
        ...
        seasonal_output_flat = self.Linear1(seasonal_flat)
        seasonal_output_flat = self.dropout1(seasonal_output_flat)
        trend_output_flat = self.Linear2(trend_flat)
        trend_output_flat = self.dropout2(trend_output_flat)
