...
if __name__ == "__main__":
    ...
    for i, ax in enumerate(axes.flat):
        data, time, meta = generator.generate_one()
        ax.plot(time, data.flatten(), 'b-', alpha=0.8)
        ax.set_title(f"Sample {i+1}: {meta['n_kernels']} kernels, len={meta['length']}")
    ...
    data, time, meta = generator.generate_one(length=104)
